...
class MrcScraper(object):
    ...
    def get_availability(self, cat_num):
        ...
        if not self.logged_in:
            self.login()
        resp = self.session.get('https://www.ptplace.com/ptp/do/pricingEntryLine?lastPageViewed=pricingEntryLine&navbartype=Store&selectedcountry=USA&viewstore=false&selectedstore=SKF&bflag=false&selectedlanguage=English')
        x = html.fromstring(resp.content)
        token = x.xpath('//input[@name="org.apache.struts.taglib.html.TOKEN"]/@value')
        new_headers = [ ('Referer', 'https://www.ptplace.com/ptp/do/pricingEntryLine?lastPageViewed=pricingEntryLine&navbartype=Store&selectedcountry=USA&viewstore=false&selectedstore=SKF&bflag=false&selectedlanguage=English')]
        ...
        self.session.headers.update(new_headers)
        resp = self.session.post('https://www.ptplace.com/ptp/do/ajaxPRCart?dispatch=pricingrequest&navbartype=Store&selectedcountry=USA&viewstore=false&selectedstore=SKF&bflag=false&selectedlanguage=English',
                                    data={'longPoll':'false',
                                    'delay':'false',
    ...
    def get_tracking(self, order_num):
        if not self.logged_in:
            self.login()
        resp = self.session.get('https://www.ptplace.com/ptp/do/orderStatusList?lastPageViewed=orderStatusList&navbartype=Store&selectedcountry=USA&viewstore=false&selectedstore=SKF&bflag=false&selectedlanguage=English')
        x = html.fromstring(resp.content)
        customer_number = x.xpath('//input[@name="lastSelectedCustomerNumber"]/@value')[0]
        new_headers = [ ('Referer', 'https://www.ptplace.com/ptp/do/orderStatusList?lastPageViewed=orderStatusList&navbartype=Store&selectedcountry=USA&viewstore=false&selectedstore=SKF&bflag=false&selectedlanguage=English')]
        self.session.headers.update(new_headers)
        ...
        for table in tables:
            try:
                ...
                tracking_number = table.xpath('.//label[contains(text(),"Carrier Reference Number")]//following-sibling::span[contains(@class,"copylink")]/a/text()')[0].strip()
...
if __name__ == '__main__':
    scraper = MrcScraper()
    # print scraper.get_availability('207SF')
    print scraper.get_tracking('S1168960')
