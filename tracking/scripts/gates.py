...
class GatesScraper(object):
    ...
    def get_tracking(self, order_num):
        if not self.logged_in:
            self.login()
        resp = self.session.get('https://www.ptplace.com/ptp/do/orderStatusList?lastPageViewed=orderStatusList&navbartype=Store&selectedcountry=USA&viewstore=false&selectedstore=Gates&bflag=false&selectedlanguage=English')
        x = html.fromstring(resp.content)
        customer_number = x.xpath('//input[@name="lastSelectedCustomerNumber"]/@value')[0]
        new_headers = [ ('Referer', 'https://www.ptplace.com/ptp/do/orderStatusList?lastPageViewed=orderStatusList&navbartype=Store&selectedcountry=USA&viewstore=false&selectedstore=Gates&bflag=false&selectedlanguage=English')]
        self.session.headers.update(new_headers)
