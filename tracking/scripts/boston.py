...
class BostonScraper(object):
    ...
    def get_tracking(self, order_num):
        if not self.logged_in:
            self.login()
        # first get their corresponding sale code in order to access the actual data
        resp = self.session.get('https://www.bostongear.com/smartcat/app/smartbuy/OrdStatBody.asp?vid=0%2C0%2C0%2C0&ordNo=&sdate=&edate=&pono=' + order_num + '&cname=&cnum=&webno=&prtno=&modno=&brand=')
        x = html.fromstring(resp.content)
        try:
            redirect_code = x.xpath(".//a[contains(@href,'javascript:ShowItems')]/text()")[0].strip()
        # no processed order code means wrong data or not yet available
        except IndexError:
            return {}
        # the page with the order data
        resp = self.session.get('https://www.bostongear.com/smartcat/app/smartbuy/ordstatusdetail.asp?ordno=%s' % redirect_code)
        results = {}
        x = html.fromstring(resp.content)
        tracking_code = x.xpath(".//a[contains(@onclick,'window.open(')]/text()")[0]
        products = x.xpath(".//table//th[contains(text(),'Part No')]/following::td[2]/text()")
        results.update(dict([(product, tracking_code) for product in products]))
        return results
