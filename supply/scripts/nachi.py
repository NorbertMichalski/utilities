...
class NachiScraper(object):
    ...
    def get_availability(self, cat_num):
        if not self.logged_in:
            self.login()
        resp = self.session.get('http://www.idc-usa.com/Default.aspx?Page=Uploaded%20Vendor%20Availability%20Report&ContentPage=TRUE&ItemCode=NAC+' + cat_num)
...
if __name__ == '__main__':
    scraper = NachiScraper()
    print scraper.get_availability('5204 ZZ')
